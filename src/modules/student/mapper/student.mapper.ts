...
export class StudentMapper {
  ...
  static detailedWithRelations(student: Student) {
      ...
      const articles = student.articles?.map((article) => ArticleMapper.detailed(article))
      const enrollments = student.enrollments?.map((enrollment) => EnrollmentMapper.detailed(enrollment))
