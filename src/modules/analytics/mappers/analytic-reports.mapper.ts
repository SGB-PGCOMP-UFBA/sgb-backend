export class AnalyticReportsMapper {
  static groupByAgencyAndYear({ agencies, scholarshipGroupedByAgencyAndYear }) {
    ...
    scholarshipGroupedByAgencyAndYear.forEach(
      ({ scholarship_year, agency_name, count }) => {
  ...
  static groupByAgencyAndCourse({
    ...
    scholarshipGroupedByAgencyAndCourse.forEach(
      ({ agency_name, course_name, count }) => {
